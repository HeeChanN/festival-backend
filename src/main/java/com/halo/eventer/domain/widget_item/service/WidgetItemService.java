...
public class WidgetItemService {
    ...
    public List<WidgetItemResDto> getWidgetItemWithWidgetId(Long widgetId) {
        ...
        return widgetItems.stream().map(WidgetItemResDto::from).collect(Collectors.toList());
    }
    ...
    public List<ImageDto> getWidgetItemImages(Long id) {
        ...
        return widgetItem.getImages().stream().map(ImageDto::from).collect(Collectors.toList());
    }

    @Transactional
    public WidgetItemResDto updateWidgetInfo(Long id, WidgetItemCreateDto widgetItemCreateDto) {
        WidgetItem widgetItem =
                widgetItemRepository.findById(id).orElseThrow(() -> new WidgetItemNotFoundException(id));
